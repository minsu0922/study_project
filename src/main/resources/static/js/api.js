...
async function fetchDomains() {
  try {
    ...
    // 분야 필터가 텅 빈다. 분야가 0개인 상태는 정상적으로 존재하지 않으므로(domain_setting 표가
    // 등록부이고, 빈 표는 기동 시 기본 분야로 한 번 채워진다. 마지막으로 켜진 분야는 끌 수도
    // 지울 수도 없다) 빈 배열은 "아직 모른다"로 읽고, 실패했을 때와 똑같이 폴백을 남긴다.
