    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // 공개: 분야 목록(8번 작업) — 문서 목록·자유 퀴즈·오답노트가 로그인 전에도
                        // 분야 필터를 그린다. 막히면 401이 나서 화면 필터가 조용히 예전 하드코딩
                        // 폴백으로만 도는데, 그 폴백은 "요청 실패에 대비한 안전망"이지 정상 경로가
                        // 아니다 — 여기를 열지 않으면 사실상 항상 폴백만 쓰이는 셈이 된다.
