...
public enum DocumentEdition {
     ...
     * 지면을 "실제로는 어디에서 만나는가"와 "언제 깨지는가"에 쓴다.
     * 중급(2일차)·고급(3일차) 문제의 근거가 된다.
     *
     * <p><b>2026-09-14에 중급이 이 편으로 옮겨 왔다.</b> 고급 3문제만 떠받치던 시절에는
     * 재료가 남아돌았고(문제당 3,465자 대 입문편 955자), 남는 지면을 모델이 이론으로 채우면서
     * 글이 계속 어려워졌다. 중급이 붙으면 "너무 어려우면 중급을 못 만든다"는 바닥이 생긴다 —
     * 프롬프트로 타이르는 것보다 구조로 막는 쪽이 잘 듣는다.
     *
     * <p><b>상수 이름을 {@code APPLIED}로 바꾸지 않은 이유</b>: 성격은 "더 어려운 편"에서
     * "쓰이는 자리를 보여 주는 편"으로 옮겨 가는 중이지만, 이 이름이 들어간 값이 어딘가에
     * 저장돼 있을 수 있다. 이름을 바꾸려면 그것부터 확인해야 하고, 그건 이번 변경과 상관없는
     * 위험이다. 뜻은 이 주석과 {@code getDisplayName}이 진다.
     */
    ADVANCED("심화편");
