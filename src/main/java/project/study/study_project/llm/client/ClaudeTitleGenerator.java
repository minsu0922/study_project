...
public class ClaudeTitleGenerator implements TitleGenerator {
     ...
     * 제목 규칙만 옮겨 오고, 그것들이 어긋나지 않는지는 테스트가 두 프롬프트를 대조한다
     * ({@code ClaudeTitleGeneratorPromptTest}).
     *
     * <p><b>2026-09-17에 문서 제목 규칙과 잣대를 맞췄다.</b> 의문형 어미 금지·줄표 부연 금지·
     * 제품 이름은 괄호를 세 곳에 나란히 넣었다(여기, 문제 생성 프롬프트, 문서 생성 프롬프트).
     * 목록 화면에서 문서와 문제가 나란히 읽히는데 제목 형식만 다르면 두 세대처럼 보인다.
     * 판정 쪽도 한 곳으로 모았다({@code TitleStyleRule}).
     *
     * <p>괄호 규칙에는 문제 쪽에만 있는 단서를 붙였다 — <b>그 이름이 정답이면 넣지 마라.</b>
     * 문서에는 정답이라는 것이 없지만, 문제 제목은 답을 가리는 것이 존재 이유의 절반이다.
     * 새 규칙을 넣을 때 기존 규칙과 부딪히는 자리를 함께 적는다는 이 프롬프트의 원칙 그대로다.
     *
    ...
    static final String SYSTEM_PROMPT = """
            ...
            - 이 문제가 무엇에 관한 것인지를 40자 이내의 <명사구>로 적는다.
              물음이 아니라 이름이다.
            - 의문형 어미로 끝내지 마라("~는가", "~인가", "~일까", 물음표).
            - 줄표(—)로 부연을 달지 마라.
            - 특정 제품·도구를 다루는 문제면 일반 개념을 앞에 쓰고 이름을 괄호에 넣는다.
              (O) 커넥션 풀이 고갈될 때의 대기 동작 (HikariCP)
              단, 그 이름 자체가 정답이면 괄호에 넣지 마라 — 목록에서 답을 알려 주는 셈이다.
              가리킬 제품이 없으면 괄호를 쓰지 않는다. 억지로 붙이지 마라.
            ...
            (X) 주문 API의 처리 건수가 1,000건 중 970건에서 멈춘 상황
                → 지문 요약이다. 상황은 주제가 아니다.
            (X) 커넥션 풀 고갈 — 대기하다 타임아웃으로 끝나는 자리
                → 줄표 부연이다. 40자 안에서 부제까지 붙이면 목록에서 잘린다.
