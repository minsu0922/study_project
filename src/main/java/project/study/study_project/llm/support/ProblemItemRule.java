...
public final class ProblemItemRule {
     ...
     * 가운데 두고 잡았다.
     *
     * <h2>2026-09-08 — 하한만 200에서 150으로 다시 내렸다</h2>
     *
     * <p>위 문단이 걱정한 상태가 <b>실제로 벌어져 있었다</b>. 개정 뒤에 나온 해설 77개를 재 보니
     * 평균 228자에 <b>27개(35%)가 200자 미만</b>이었다. 셋에 하나꼴로 뜨는 경고이고, 그 정도면
     * 목록 전체가 안 읽힌다({@code rationaleWarningsOf}의 "옛 형식입니다" 경고를 되돌린 일).
     *
     * <p><b>더 중요한 것은 검수자가 할 수 있는 일이 없다는 점이다.</b> 27개가 제각기 얇은 것이
     * 아니라 모델이 하한에 붙어서 나온다 — 그건 문제 하나의 결함이 아니라 프롬프트와 모델
     * 사이의 계통 오차다. 항목마다 경고를 다는 것은 <b>같은 사실을 27번 말하는 것</b>이고,
     * 사람이 승인 화면에서 60자를 채워 넣어 고칠 종류가 아니다.
     *
     * <p><b>150인 근거도 실측이다.</b> 같은 77개의 분포가 125~149에 6개, 150~199에 21개다.
     * 150에 그으면 6개(8%)만 남고 — 열둘에 하나라 읽힌다 — 그 여섯은 실제로 정답 근거 한 문단이
     * 안 되는 얇은 해설이다. 반대로 150~199 구간의 스물하나는 근거와 문서 안내가 다 들어 있다.
     *
     * <p><b>상한 400은 건드리지 않았다.</b> 같은 77개 중 400자를 넘긴 것이 <b>하나도 없다</b>
     * (개정 전에는 67%였다). 헛울리지 않으면서 되돌아갈 자리를 지키는 중이라 그대로 둔다.
     */
    public static final int EXPLANATION_MIN = 150;
