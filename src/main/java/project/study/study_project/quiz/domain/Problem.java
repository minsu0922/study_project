...
public class Problem {
    ...
    /**
     * 제목만 붙인다 — 제목 백필 전용(V13).
     *
     * <p><b>{@link #update}를 쓰지 않는 이유</b>: 그쪽은 "폼 전체 제출" 방식이라 내용 필드를
     * 통째로 교체한다. 백필은 이름 하나만 채우는 일인데 그 메서드를 쓰면 나머지 일곱 개를
     * 다시 읽어 그대로 되넣어야 하고, 그러다 하나라도 빠뜨리면 <b>멀쩡한 지문이 지워진다</b>.
     * 하는 일이 좁으면 메서드도 좁은 편이 안전하다.
     *
     * <p><b>이미 제목이 있으면 덮어쓰지 않는다.</b> 백필 대상은 조회 조건이 이미 걸러 주지만,
     * 여기서 한 번 더 막는다 — 사람이 손으로 지은 제목을 다음 번 백필이 모델 제목으로
     * 갈아 치우는 것은 되돌릴 방법이 없다. 조회와 저장 사이의 시차로도 충분히 생길 수 있는 일이다.
     *
     * @return 실제로 채웠으면 {@code true}. 부르는 쪽이 "몇 건 채웠는지"를 셀 수 있게 한다
     */
    public boolean fillTitleIfAbsent(String newTitle) {
        if (this.title != null || newTitle == null || newTitle.isBlank()) {
            return false;
        }
