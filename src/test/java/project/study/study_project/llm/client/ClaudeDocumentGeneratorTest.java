...
class ClaudeDocumentGeneratorTest {
    ...
    /**
     * "어디서 만나는가" 절이 <b>실무 위치</b>를 묻는 자리로 남아 있는지 본다.
     *
     * <p>8/15 실물을 끝까지 읽어도 이 개념을 코드 어디에서 만나는지 알 수 없었다. 육하원칙 중
     * 유일하게 비어 있던 칸이라 절로 만들었는데, 이 절은 방치하면 <b>본론의 요약으로 변질되기
     * 쉽다</b> — 모델 입장에서 원리를 한 번 더 쓰는 게 설정 이름을 정확히 대는 것보다 안전하다.
     * 그래서 "원리를 다시 설명하지 마라"와 "확실하지 않으면 적지 마라"를 함께 못 박는다.
     * 뒤엣것이 특히 중요하다 — 기본값은 그럴듯한 거짓이 나오기 딱 좋은 자리다.
     */
    @Test
    @DisplayName("'어디서 만나는가'는 실무 위치를 묻는 절이다 — 본론 요약으로 변질되지 않게 못 박는다")
    void pinsWhereYouMeetItSection() {
        assertThat(ClaudeDocumentGenerator.SYSTEM_PROMPT)
                .contains("## 어디서 만나는가")
                .as("설정 자리·기본값·에러 메시지 이름이 이 절의 알맹이다")
                .contains("대표 기본값")
                .as("모델은 확신 없는 기본값을 그럴듯하게 지어낸다 — 비우는 편이 낫다고 못 박는다")
                .contains("확실하지 않은 설정 이름·기본값은 아예 적지 마라")
                .as("방치하면 본론 요약이 된다")
