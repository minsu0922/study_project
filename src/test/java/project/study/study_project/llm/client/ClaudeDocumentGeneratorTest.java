...
class ClaudeDocumentGeneratorTest {
    ...
    void topicPickingExamplesStayNeutral() {
        ...
        // 2026-09-04: [제목] 예시만 스레드로 남아 있었다. 예시 주제에는 "가져다 쓰지 마라"가
        // 따라붙는데, 그러면 <그 주제를 지정한 요청>과 정면으로 부딪힌다 — 실제로 스레드
        // 주제를 다시 뽑으려다 걸렸다. 예시가 규칙을 이기므로 금지도 함께 실려 나간다.
        // 세 예시 블록이 모두 캐시를 쓰는 것은 우연이 아니라 <한 주제로 몰아 두려는> 것이다.
    ...
    /* ── 2026-09-16: 글 전체가 하나로 읽히게 하는 규칙 둘 ────────── */

    /**
     * <b>절과 절 사이에 다리를 놓게 한다.</b>
     *
     * <p>사용자 지적은 둘이었다. "절과 절 사이가 툭툭 끊긴다"와 "글 전체가 무엇을 향해
     * 가는지 모르겠다". 원인 쪽이 중요하다 — 이 프롬프트는 흐름을 <b>일부러</b> 깎아 놨다.
     * {@code [덜어낼 것]}이 연결어("정리하면", "즉")를 이름으로 금지한다. 2026-08-15에
     * 분량 상한을 올리면서 넣은 규칙인데, 늘어난 지면이 부연으로 채워지는 것을 막으려고
     * "담백함"을 얻고 "이어짐"을 판 것이다.
     *
     * <p>그래서 "매끄럽게 써라"를 한 줄 더하는 것으로는 안 된다. 금지 목록과 정면으로
     * 부딪쳐 모델이 둘 중 하나를 조용히 무시하고, 어느 쪽을 무시했는지는 사람이 글을 다
     * 읽어야 안다. 대신 <b>자리</b>를 요구한다 — 각 절이 문장 하나로 시작하면 다리가
     * 들어설 칸이 생기고, 그 칸이 있는지는 검증기가 원문에서 셀 수 있다.
     *
     * <p>요약을 쓰라는 것이 아님을 함께 적는 것이 핵심이다. 그냥 "앞 절을 받아라"라고만
     * 하면 모델은 "앞에서 ~를 봤다. 이제 ~를 보자"를 쓰는데, 그건 {@code [덜어낼 것]}의
     * 예고·되풀이에 그대로 걸린다. 두 규칙이 서로를 무효로 만드는 자리라 못 박아 둔다.
     */
    @Test
    @DisplayName("절을 문장으로 시작하라는 지시가 두 편에 모두 실린다 — 목록이 곧장 시작하면 다리가 들어설 칸이 없다")
    void bothPromptsRequireABridgeSentencePerSection() {
        for (String prompt : List.of(ClaudeDocumentGenerator.BEGINNER_SYSTEM_PROMPT,
                ClaudeDocumentGenerator.ADVANCED_SYSTEM_PROMPT)) {
            assertThat(prompt)
                    .as("자리를 요구하지 않으면 굵은 항목이 절 첫 줄부터 시작한다")
                    .contains("[절과 절을 잇는다]")
                    .contains("목록·표·코드가 아니라 문장 하나로 시작한다")
                    .as("요약을 시키면 [덜어낼 것]의 예고·되풀이와 정면으로 부딪친다")
                    .contains("앞 절을 요약하지 마라");
        }
    }
     ...
     * 이 프롬프트는 자기 {@code [덜어낼 것]}을 따로 썼다. 그래서 답변에서는 금지된 것이
     * 문서에는 그대로 나왔다 — 실측하니 {@code ~하는 것} 21건, {@code ~할 수 있다} 16건,
     * {@code ~에 대해} 13건, {@code 것이다} 10건이었다.
     *
     * <p>반대로 <b>이미 두 곳 모두에 있던 규칙</b>은 잘 듣고 있었다. 이중 피동·{@code ~에 의해}·
     * {@code ~을 통해}·{@code 가지다}는 3,678문장에서 0~1건이다. 규칙을 한 곳에 모으는 것이
     * 문구를 더 세게 쓰는 것보다 잘 듣는다는 증거라, 없는 쪽에 마저 적는다.
     *
     * <p>줄표 규칙만 새로 정했다. 문장 <b>가운데</b>를 줄표로 끊고 정의를 끼워 넣는 자리가
     * 문장을 가장 길게 만든다(146자·137자 실물). 줄표 자체를 금지하지 않은 이유는 그것이
     * 이 문서들의 글맛이기 때문이다 — 자리만 문장 끝으로 옮긴다.
     */
    @Test
    @DisplayName("답변용 문체 규칙이 두 편의 [덜어낼 것]에도 실린다 — 규칙이 두 곳에 갈려 있었다")
    void bothPromptsCarryTheAnswerStyleRules() {
        for (String prompt : List.of(ClaudeDocumentGenerator.BEGINNER_SYSTEM_PROMPT,
                ClaudeDocumentGenerator.ADVANCED_SYSTEM_PROMPT)) {
            assertThat(prompt)
                    .as("번역투 — 문서에 13건 나왔다")
                    .contains("\"~에 대해/~에 대한\"")
                    .as("형식명사 — 문서에 31건 나왔다(~하는 것 21, 것이다 10)")
                    .contains("\"~하는 것\"")
                    .as("가능 표현 남발 — 문서에 16건 나왔다")
                    .contains("\"~할 수 있다\" 남발")
                    .as("줄표는 문장 끝에서만 — 문장 가운데를 끊는 정의가 문장을 가장 길게 만든다")
                    .contains("줄표는 문장 끝에서만 쓴다");
        }
    }

    /**
     * <b>문장 길이에 상한을 준다</b>(2026-09-16).
     *
     * <p>나간 문서 22편의 문장 3,678개를 재서 정했다. 중앙값은 42자로 멀쩡한데 <b>8%(299개)가
     * 80자를 넘고 최장이 149자</b>였다. 원인은 프롬프트에 있었다 — {@code [문체]}에 "한 문단은
     * 3문장"만 있고 <b>문장 길이 규칙이 아예 없다</b>. 100자짜리 세 문장으로 된 문단이
     * 규칙상 합격이었던 셈이다.
     *
     * <p>긴 문장의 정체도 셌다. 대부분 <b>둘을 한 문장에서 가른 것</b>이다(149자 실물:
     * "오버로딩은 …이고, 오버라이딩은 …이다"). 그래서 상한만 걸지 않고 <b>갈 곳</b>을 함께
     * 준다 — 둘을 가르는 문장은 표로 옮기라고. 이 저장소가 여러 번 확인한 것이,
     * 금지만 하면 모델이 다른 곳으로 새고 갈 곳을 주면 지킨다는 것이다.
     */
    @Test
    @DisplayName("문장 60자 상한과 '둘을 가르면 표로'가 두 편에 모두 실린다")
    void bothPromptsCapSentenceLength() {
        for (String prompt : List.of(ClaudeDocumentGenerator.BEGINNER_SYSTEM_PROMPT,
                ClaudeDocumentGenerator.ADVANCED_SYSTEM_PROMPT)) {
            assertThat(prompt)
                    .as("상한이 없으면 100자짜리 세 문장도 '한 문단 3문장' 규칙을 통과한다")
                    .contains("한 문장은 60자까지다")
                    .as("상한만 걸고 갈 곳을 안 주면 모델은 쉼표로 이어 붙여 상한을 피한다")
                    .contains("둘 이상을 가르는 문장은 표로 옮겨라");
        }
    }

    /**
     * <b>처음에 던진 질문을 끝에서 회수하게 한다.</b>
     *
     * <p>"글의 목적지가 안 보인다"의 정체가 여기였다. 두 편 모두 이미
     * {@code > 이 글이 답하는 질문:} 한 줄을 쓰고 있는데, <b>그 뒤로 아무도 그 질문을 다시
     * 보지 않는다</b>. 1쪽에서 약속하고 어디서도 갚지 않으니, 끝까지 읽어도 "이 글이 답한 게
     * 뭐였지"가 남는다.
     *
     * <p>그래서 장치를 새로 만들지 않았다. 이미 있는 질문 줄을 마지막 절이 받게 했을 뿐이다.
     * 새 개념을 더하면 프롬프트가 길어지고, 긴 프롬프트에서 먼저 무시되는 것은 늘 뒤쪽이다.
     */
    @Test
    @DisplayName("처음 던진 질문을 마지막 절이 답하게 한다 — 지금은 1쪽에서 약속하고 아무도 갚지 않는다")
    void bothPromptsCloseTheOpeningQuestion() {
        assertThat(ClaudeDocumentGenerator.BEGINNER_SYSTEM_PROMPT)
                .as("입문편의 마지막 절")
                .contains("## 한 줄로 정리하면")
                .contains("이 글이 답하는 질문에 그대로 답이 되어야 한다");
